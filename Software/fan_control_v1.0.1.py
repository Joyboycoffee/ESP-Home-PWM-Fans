...
LHM_CHECK_INTERVAL = 15           # 15 seconds for LHM checks (Renamed from OHM)
...
class OptimizedAsyncioWorker(QObject):
    ...
    def reset_reconnect_cooldown(self):
        """Reset the reconnect cooldown to allow immediate reconnection."""
        self._last_reconnect_attempt = 0
        self._reconnect_cooldown = 0

    async def async_connect(self, parent_controller=None, force=False):
        """Creates and connects the ESPHome client with improved retry logic.
        
        Args:
            parent_controller: The parent controller instance
            force: If True, bypasses the cooldown check for manual reconnects
        """
        current_time = time.time()
        # Allow bypassing cooldown for manual reconnects
        if not force and current_time - self._last_reconnect_attempt < self._reconnect_cooldown:
            return
...
class OptimizedFanController(QObject):
    ...
    async def get_cpu_gpu_temps(self):
        """Optimized temperature fetching with intelligent caching (LHM Parsing)."""
        current_time = time.time()
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.get(TEMP_DATA_URL) as response:
                    if response.status == 200:
                        ...
                        # --- START LHM PARSING LOGIC ---
                        # This logic successfully navigates the Libre Hardware Monitor JSON structure
                        for computer in data.get('Children', []):
                            for hardware in computer.get('Children', []):
                                if 'CPU' in hardware.get('Text', '') or 'Ryzen' in hardware.get('Text', ''):
                                    for sensor_group in hardware.get('Children', []):
                                        if sensor_group.get('Text') == 'Temperatures':
                                            for sensor in sensor_group.get('Children', []):
                                                if 'Package' in sensor.get('Text', '') or 'Tctl/Tdie' in sensor.get('Text', ''):
                                                    temp_str = sensor.get('Value', '0.0').split(' ')[0]
                                                    try:
                                                        temp = float(temp_str)
                                                    except ValueError:
                                                        continue
                                                    break # Found temperature, exit sensor loop
                                            if temp is not None:
                                                break # Exit sensor group loop
                                    if temp is not None:
                                        break # Exit hardware component loop
                            if temp is not None:
                                break # Exit computer loop
                        # --- END LHM PARSING LOGIC ---

                        if temp is not None:
                            # CRITICAL FIX: Cache the result ONLY after successful fetch
                            self._temp_cache = temp
                            self._temp_cache_time = current_time
                            
                            # Update GUI that LHM is working
                            if self.main_window:
                                self.main_window.update_lhm_status(True)
                            
                            print(f"Found CPU temperature: {temp}°C")
                            return temp
                        
                        print("CPU temperature not found in data")
                        # Update GUI that LHM is not working properly
                        if self.main_window:
                            self.main_window.update_lhm_status(False)
                        return None
                    else:
    ...
    async def main_loop(self):
        ...
        # Periodic connection health check (Logic remains the same)
        current_time = time.time()
        ...
        try:
            ...
            if self.automation_enabled:
                ...
                if cpu_temp is not None:
                    ...
                    # Process temperature and calculate target speed
                    target_speed = 0
                    best_match_temp = 0 # Variable to show the threshold that was met
                    ...
                    # --- CRITICAL FIX: ALWAYS UPDATE GUI HERE (Guarantees Live Temperature/Logic) ---
                    if self.main_window:
                        self.main_window.update_logic(cpu_temp, best_match_temp, target_speed)
                        self.main_window.update_cpu_temperature(cpu_temp)
                    
                    print(f"Processing temperature: {cpu_temp}°C -> Target: {target_speed}%")
...
class OptimizedFanGUI(QMainWindow):
    ...
    def __init__(self, controller, parent=None):
        ...
        self._lhm_status_cache = None 
        self._lhm_status_cache_time = 0 
    ...
    def update_status_with_settings(self, message):
        base_status = "Libre Hardware Monitor working properly" 
    ...
    def update_logic(self, temp, threshold, speed):
        ...
        if self.controller.automation_enabled:
              # Only update if a threshold was actually used (threshold > 0 means automation ran)
              if threshold > 0:
                self.logic_label.setText(f"Logic: {temp}°C ≥ {threshold}°C → Fan: {speed}%")
              else:
                 # If no threshold was met, show current temp and target speed 0
                self.logic_label.setText(f"Logic: {temp}°C < Min Threshold → Fan: 0%")
        else:
            self.logic_label.setText(f"Logic: Waiting for automation")
    
    def update_lhm_status(self, is_working): 
        """Update LHM status from temperature fetching results"""
        current_time = time.time()
        self._lhm_status_cache = is_working 
        self._lhm_status_cache_time = current_time 
        
        if is_working:
            self.update_status("Libre Hardware Monitor working properly") 
        else:
            self.update_status("Please run Libre Hardware Monitor to fetch temperatures from your PC") 
    
    def check_lhm_status(self): 
        """Initial check for Libre Hardware Monitor accessibility"""
        current_time = time.time()
        if (self._lhm_status_cache is None or 
           current_time - self._lhm_status_cache_time > LHM_CHECK_INTERVAL): 
            
            
            try:
                import urllib.request
                try:
                    # The synchronous check is fragile but needed for fast initial feedback
                    urllib.request.urlopen("http://localhost:8085/data.json", timeout=1)
                    self.update_lhm_status(True) 
                except:
                    self.update_lhm_status(False) 
                self._lhm_status_cache_time = current_time 
            except:
                self.update_lhm_status(False) 
                self._lhm_status_cache_time = current_time 
    ...
    def setup_tray_icon(self):
        ...
        tray_menu.addSeparator()
        self.connect_action = QAction("Reconnect to ESP", self)
        self.connect_action.triggered.connect(self.connect_from_tray)
    ...
    def connect_from_tray(self):
        """Reconnect to ESP through tray menu action."""
        ...
        if not self.validate_ip_address(ip_address):
            self.tray_icon.showMessage(
                "ESP Home Fan Controller",
                "Invalid IP address. Please check the IP in the main window.",
        ...
        # Reset reconnect cooldown to allow immediate reconnection
        if self.controller.worker:
            self.controller.worker.reset_reconnect_cooldown()
        ...
        # Trigger reconnection with force=True to bypass cooldown
        if self.controller.worker:
            self.controller.worker.schedule_task(
                self.controller.worker.async_connect(self.controller, force=True)
            )

        # Visual feedback in tray
        self.tray_icon.showMessage(
            "ESP Home Fan Controller",
            f"Connecting to ESP device at {ip_address}...",
            QSystemTrayIcon.MessageIcon.Information,
            2000
        )
    ...
    def connect_to_esp_device(self):
        ...
        # Trigger reconnection with new IP (force=True bypasses cooldown)
        if self.controller.worker:
            self.controller.worker.schedule_task(self.controller.worker.async_connect(self.controller, force=True))
